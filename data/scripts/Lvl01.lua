...
function checkExec(exec, is_playback)
	if
		exec.Ent
		and exec.Ent.Side.Intruder
		and GetDistanceBetweenEnts(exec.Ent, store.Waypoint1) <= 3
		and not store.nFirstWaypointDown
	then
	...
	if store.nFirstWaypointDown then
		if
			exec.Ent
			and exec.Ent.Side.Intruder
			and GetDistanceBetweenEnts(exec.Ent, store.Waypoint2) <= 3
			and not store.nSecondWaypointDown
		then
...
function StoreWaypoint(wpname, wpside, wppos, wpradius, wpremove)
	waypoint_exec =
		{ script_waypoint = true, name = wpname, side = wpside, pos = wppos, radius = wpradius, remove = wpremove }
